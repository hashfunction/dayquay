...
# Translators: noun
preview = _("Preview")
preview1 = _("There are two modes in Jotmorrow, the __edit__ mode and the __preview__ mode.")
...
temp1 = _("Jotmorrow supports templates.")
...
Jotmorrow lets you insert images, files and links into your entries.
...
the template files from inside Jotmorrow by opening the menu next to
the "Template" button.
...
left. Its contents are only refreshed when Jotmorrow starts and when
...
Jotmorrow supports spellchecking your entries. This feature needs the
...
Use Hunspell dictionaries (not Aspell) with Jotmorrow on Windows:

+ Download the desired language files (.aff and .dic) from a Hunspell source, e.g., https://github.com/wooorm/dictionaries.
+ Place both files into <Jotmorrow Dir>\\share\\enchant\\myspell\\.
+ Restart Jotmorrow and pick the language via right-click → Languages in the editor.
...
Syncing Jotmorrow with a remote server is easy. You can either use a
...
time you open Jotmorrow you will find your server in the "New",
...
Using Jotmorrow from multiple operating systems on the same computer
...
Jotmorrow can be run in portable mode. In this mode, the template
...
To use Jotmorrow on a flash drive on Windows, run the installer and
...
In recent Jotmorrow versions you can export your journal directly to
...
Jotmorrow data will be encrypted, too.


== Dark Mode ==

Jotmorrow follows your system's theme settings by default. Here are
...
for GTK applications like Jotmorrow:
...
2. **Using environment variables:**
   ...
   - Start Jotmorrow from terminal: ``GTK_THEME=Adwaita-dark jotmorrow``
   - Or add to your shell profile to make it permanent

=== Flatpak installations ===

For Jotmorrow installed via Flatpak:
...
Jotmorrow installation. Once you find a GTK3 compatible theme, copy
the theme into ``<Jotmorrow Dir>\\share\\themes``, e.g.,
``C:\\Program Files (x86)\\Jotmorrow\\share\\themes`` and then edit
``C:\\Program Files (x86)\\Jotmorrow\\etc\\gtk-3.0\\settings.ini`` to
...
``<Jotmorrow Dir>\\share\\themes``. Then edit
...
reinstalling Jotmorrow. However, this might change the theme of every GTK
...
START /D ^"C:\\Program Files (x86)\\Jotmorrow^" Jotmorrow.exe"
```
...
Jotmorrow supports mathematical formulas that render in preview (via MathJax)
...
The content of a Jotmorrow journal is saved in a directory with many
...
me a note. This way Jotmorrow can get better, not only for you, but
for all users.
