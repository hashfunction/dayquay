...
def artifact(number,name,root,max_bytes,exact_members=None):
    endpoint=f'repos/hashfunction/dayquay/actions/artifacts/{number}'
...
def main(output,qualified):
    ...
    checks.require(sys.platform=='win32' and os.environ.get('CI')=='true'
        and os.environ.get('GITHUB_REPOSITORY')=='hashfunction/dayquay','Screenshot inputs require isolated Jotmorrow Windows CI')
    checks.assert_qualified_checkout(qualified)
    checks.msix._reject_link(output.parent);output.mkdir()
    run=gh_json(f'repos/hashfunction/dayquay/actions/runs/{checks.RUN}')
    store=output/'store';metadata=output/'metadata'
    package_artifact=artifact(10294266549,'Jotmorrow-unsigned-store-package',store,50000000,[checks.PACKAGE_NAME,'release-ready.json'])
    metadata_artifact=artifact(10294785652,'Jotmorrow-native-qualification',metadata,16000000)
