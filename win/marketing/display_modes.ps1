...
function Add-MarketingDisplayTypes {
    if('JotmorrowMarketing.DisplayModes' -as [type]){return}
...
function Get-MarketingDisplayState([string]$Device){
    ...
    if(-not $Device){$Device=[JotmorrowMarketing.DisplayModes]::Primary()}
    return @{device=$Device;current=[JotmorrowMarketing.DisplayModes]::Current($Device);modes=[JotmorrowMarketing.DisplayModes]::Supported($Device)}
}
function Set-MarketingDisplayMode([string]$Device,$Mode,[int]$Flags){
    return [JotmorrowMarketing.DisplayModes]::Change($Device,$Mode,$Flags)
}
