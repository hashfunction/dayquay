...
$script:probeRoot=Join-Path ([IO.Path]::GetTempPath()) ('dayquay-temporary-test-'+[guid]::NewGuid().ToString('N'))
...
try {
    $failure=$null
    try { Invoke-DayQuayInstallQualification unused unused unused $script:probeRoot | Out-Null }
