...
$script:ActualCore = ${function:Invoke-DayQuayQualificationCore}
...
foreach ($scenario in @('observation-empty','failed-add-race','ambiguous-add','wrong-architecture','observation-failed','owned','owned-with-foreign','remove-failed','normal-owned','normal-with-foreign')) {
    ...
    try {
        ...
        $failure=$null
        try { Invoke-DayQuayInstallQualification unused unused unused $temporary | Out-Null } catch { $failure=$_.Exception.Message }
        $fixture=$global:RegistrationFixture
        ...
        if ($scenario -eq 'observation-empty') {
            if ($fixture.removed.Count -or $fixture.registrations.Count -or $evidence.registration_ownership_established) { throw 'Invisible registration must not establish cleanup ownership' }
            if (-not $failure -or -not $evidence.add_appx_completed -or -not $evidence.primary_error -or $evidence.installation_qualification_passed) { throw 'Successful Add and failed observation evidence lost' }
            if ($evidence.cleanup_errors.Count -ne 1 -or $evidence.cleanup_errors[0] -notmatch 'uncertain') { throw 'Never-observed registration cleanup uncertainty missing' }
