...
1ee1e1849ad2aad030717b7ba56254197436ea4e and ReticleQuay;
...
from window_title_contract import (
    SOURCE_FILES as TITLE_SOURCE_FILES,
    create_title_contract,
...
REQUIRED_RELEASE_FILES = tuple(RUNTIME.values()) + (
    ...
    "_internal/gi_typelibs/GtkSource-4.typelib",
    "_internal/share/hunspell/en_US.aff",
    "_internal/share/hunspell/en_US.dic",
    "_internal/LICENSE",
...
BUILD_ARTIFACTS = (
    ...
    "native-downloaded-versions.txt",
    "msys2-cache-sha256.txt",
    "msys2-package-metadata.txt",
    "msys2-package-files.txt",
    "native-runtime.json",
    "native-notices.json",
    "packaging-python.json",
)
...
def source_inputs(source_root):
    ...
    titles = [
        ...
        and any(
            isinstance(target, ast.Name) and target.id == "program_name" for target in node.targets
        )
    ]
    window = ast.parse((source_root / "rednotebook/gui/main_window.py").read_text(encoding="utf-8"))
    calls = [
        node
    ...
    if (
        ...
        or not (
            isinstance(calls[0].args[0], ast.Attribute)
            and calls[0].args[0].attr == "program_name"
            and isinstance(calls[0].args[0].value, ast.Name)
    ...
    result = {
        ...
        for name in (
            ...
            *TITLE_SOURCE_FILES,
            ...
            "win/rednotebook.spec",
            "win/build_support.py",
            "win/dayquay-runtime-hook.py",
            "win/dayquay-runtime-probe.py",
            "win/hooks/hook-gi.repository.GIRepository.py",
            "win/hooks/hook-gi.repository.GtkSource.py",
        )
    }
...
def _versions(path):
    ...
    for line in Path(path).read_text(encoding="utf-8").splitlines():
        ...
        if (
            ...
            or not parts[0].startswith("mingw-w64-ucrt-x86_64-")
        ):
            raise ValueError("Invalid/duplicate installed native package version")
        values[parts[0]] = parts[1]
    if not values:
        raise ValueError("Empty native package version record")
    return values


def native_build_inputs(source_root):
    source_root = Path(source_root)
    evidence = source_root / "build-evidence"
    artifacts = {name: file_record(evidence / name) for name in BUILD_ARTIFACTS}
    installed = _versions(evidence / "native-installed-versions.txt")
    if installed != _versions(evidence / "native-downloaded-versions.txt"):
        raise ValueError("Downloaded native package versions differ from installed versions")
    ...
    if {name: value["sha256"] for name, value in cached.items()} != declared:
        raise ValueError("Native package cache differs from recorded hashes")
    declaration = _load_json(
        source_root / "win/windows-dependencies.json", "native dependency declaration"
    )
    for package in declaration["packages"]:
        ...
        if (
            installed.get(package["name"]) != package["version"]
            or declared.get(archive) != package["sha256"]
        ):
            raise ValueError(
                "Native input differs from source-qualified dependency: " + package["name"]
            )
    native = _load_json(evidence / "native-runtime.json", "native runtime receipt")
    if (
        native.get("status") != "native_runtime_qualified_not_packaged"
        or native.get("gtk", [None])[0] != 3
        or native.get("spellcheck") != "en_US correct and incorrect words verified"
    ):
        raise ValueError("Real GTK3/spellcheck runtime receipt is absent or failed")
    return {"artifacts": artifacts, "archives": cached, "installedVersions": installed}
...
def create_input_inventory(release, source_root, source_commit):
    ...
    if len(bindings) != 1:
        raise ValueError("Expected exactly one native PyGObject _gi binding")
    runtime = dict(RUNTIME, pygobject=bindings[0])
    sources = source_inputs(source_root)
    copies = dict(SOURCE_COPIES)
    for tree, target in SOURCE_TREES.items():
        copies.update(
            {name: target + name[len(tree) :] for name in sources if name.startswith(tree + "/")}
        )
    for original, staged in copies.items():
        if files.get("_internal/" + staged) != sources[original]:
            raise ValueError(
                f"Original source notice/artwork/resource changed or missing: {original}"
            )
    build = native_build_inputs(source_root)
    notices = _load_json(Path(source_root) / "build-evidence/native-notices.json", "native notices")
    if (
        not isinstance(notices.get("files"), dict)
        or not notices["files"]
        or not notices.get("unresolved")
        or notices.get("inventoryIsLicenseClearance") is not False
    ):
        raise ValueError(
            "Native notices must include collected files and explicit unresolved release audit"
        )
    if inventory_tree(release / "_internal/notices") != notices["files"]:
        raise ValueError("Collected native notice inventory differs from staged bytes")
    return dict(
        schemaVersion=1,
        ...
        runtime=runtime,
        unresolvedNotices=notices["unresolved"],
        buildProvenance=dict(
            bootloader="PyInstaller embedded bootloader build output; not package-archive-identical",
            dependencies="MSYS2 exact installed/downloaded versions, hashed cache and source-qualified direct inputs",
            resolution="Existing rolling MSYS2 setup; not a reproducible dependency resolver",
            nativeInputs=build,
...
def validate_input_evidence(release, inventory, startup, source_root, source_commit):
    ...
    if recorded != measured:
        raise ValueError(
            "Full stage inventory or source metadata differs from current source-bound inputs"
        )
    receipt = _load_json(startup, "same-run native startup receipt")
    expected_title = validate_title_contract(source_root, receipt.get("window_title_contract"))
    if (
        ...
        raise ValueError(
            "Native startup receipt does not bind this exact source, inventory and executable"
        )
...
def stage_release(release, artwork, stage, source_commit, inventory, startup, source_root):
    ...
    if "AppxManifest.xml" in input_inventory or any(
        name.casefold().startswith("assets/") for name in input_inventory
    ):
        raise ValueError("Release collides with qualification-owned manifest/assets")
    input_record, startup_record = validate_input_evidence(
        release, inventory, startup, source_root, source_commit
    )
    with _regular_stream(artwork) as stream:
        artwork_data = stream.read()
    artwork_record = {
        "bytes": len(artwork_data),
        "sha256": hashlib.sha256(artwork_data).hexdigest(),
    }
    assets = {}
    ...
    try:
        ...
        return {
            ...
            "windowTitleContract": startup_record["window_title_contract"],
            "sourceInputs": input_record["sourceInputs"],
            "runtime": input_record["runtime"],
    ...
    with zipfile.ZipFile(path) as archive:
        for info in archive.infolist():
            ...
            if info.is_dir():
                ...
                if (
                    stat.S_IFMT(mode) not in (0, stat.S_IFDIR)
...
def verify_record_inputs(
    ...
    with tempfile.TemporaryDirectory(prefix="dayquay-record-") as temporary:
    ...
    if (
        ...
        or type(unpacked.get("verifiedPayloadFiles")) is not int
...
def _tool_record(path, sdk_version):
    ...
    if normalized[-6:] != expected_tail:
        raise ValueError(
            "MakeAppx SDK path must end with Windows Kits/10/bin/<exact-version>/x64/makeappx.exe"
        )
    with _regular_stream(path) as stream:
...
def build_qualification(
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".dayquay-msix-", dir=output.parent))
    try:
        ...
        output.mkdir()
        try:
            with _regular_stream(package) as source, (output / package.name).open(
                "xb"
            ) as destination:
            ...
            _write_new(
                output / "INCOMPLETE.txt",
                b"Packaging output is incomplete and must not be consumed.\n",
            )
            raise
        ...
        raise ValueError(
            f"Qualification packaging failed; evidence retained at {temporary}: {error}"
        ) from error
...
def main():
    ...
    try:
        ...
        if actual != args.source_commit:
            raise ValueError(
                f"Source commit mismatch: expected {args.source_commit}, found {actual}"
            )
        ...
        if dirty:
            raise ValueError(
                "Source checkout must be clean so the recorded commit identifies every packaging input"
            )
        print(
            build_qualification(
                args.release,
