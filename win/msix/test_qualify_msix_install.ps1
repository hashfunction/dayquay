# Failure-path tests for the real DayQuay qualification orchestration.
...
$insidePackage = Test-PathInside -Candidate (Join-Path $packageRoot 'DayQuay.exe') -Root $packageRoot
...
$recordFixture = [pscustomobject]@{ payload = [pscustomobject]@{ 'DayQuay.exe' = [pscustomobject]@{ bytes=1; sha256=('a' * 64) } } }
Assert-True ((Get-RecordPayloadEntry $recordFixture 'DayQuay.exe').bytes -eq 1) 'slash-qualified payload property must resolve exactly'
