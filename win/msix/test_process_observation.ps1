...
foreach ($code in @(0,7)) {
    ...
    try {
        ...
        $timeout=Get-DayQuayProcessExitEvidence $attached 1
        ...
        $exit=Get-DayQuayProcessExitEvidence $attached 10000
...
try {
    $observed=Get-DayQuayProcessExitEvidence $unattached 1
...
foreach ($owned in @($false,$true)) {
    $directory=Join-Path ([IO.Path]::GetTempPath()) ('dayquay-process-ownership-'+[guid]::NewGuid().ToString('N'))
