...
if ($exeHash -cne $inventory.files.'DayQuay.exe'.sha256) { throw 'Native executable differs from stage inventory.' }
...
try {
  ...
  if ($process.MainWindowHandle -eq 0 -or -not (Test-DayQuayWindowTitle $process.MainWindowTitle $expectedTitle)) {
    throw "Expected the native DayQuay main window, got: $($process.MainWindowTitle)"
  }
  Start-Sleep -Seconds 3
  $process.Refresh()
  if ($process.HasExited -or $process.MainWindowHandle -eq 0 -or -not (Test-DayQuayWindowTitle $process.MainWindowTitle $expectedTitle)) { throw 'DayQuay did not retain its exact stable main window.' }
  ...
  $receipt = @{ source_commit=$env:GITHUB_SHA; generated_at_utc=[DateTime]::UtcNow.ToString('o'); package_inventory_sha256=$inventoryHash; windows_native_startup=$true; window_title=$process.MainWindowTitle; window_title_contract=$titleContract; executable_sha256=(Get-FileHash $executable -Algorithm SHA256).Hash; interactive_backup_restore_verified=$false; native_source_clearance=$false; msix_built=$false; submitted=$false }
